...
`ifdef FORMAL
    // ── PLIC priority encoder invariants ─────────────────────────────
    // P_WINNER_BOUND:   winner_id always in valid range
    // P_WINNER_ACTIVE:  if there is a winner, it must be an active source
    // P_WINNER_OPTIMAL: every active source has priority ≤ win_pri
    //                   (winner holds the globally highest priority level)
    // P_MEIP:           meip tracks whether a winner exists
    ...
    always @(posedge clk) begin
        if (rst_n) begin
            ...
            if (winner_id != 3'd0) begin
                // P_WINNER_ACTIVE: winner must be in src_active
                case (winner_id)
                    3'd1: assert(src_active[1]);
                    3'd2: assert(src_active[2]);
                    3'd3: assert(src_active[3]);
                    3'd4: assert(src_active[4]);
                    3'd5: assert(src_active[5]);
                    3'd6: assert(src_active[6]);
                    default: assert(1'b0);  // unreachable given BOUND
                endcase

                // P_WINNER_OPTIMAL: no active source outranks the winner
                if (src_active[1]) assert(pri1 <= win_pri);
                if (src_active[2]) assert(pri2 <= win_pri);
                if (src_active[3]) assert(pri3 <= win_pri);
                if (src_active[4]) assert(pri4 <= win_pri);
                if (src_active[5]) assert(pri5 <= win_pri);
                if (src_active[6]) assert(pri6 <= win_pri);
            end

            // P_MEIP: interrupt flag ↔ winner exists
