            ...
            case (tx_state)
                TX_IDLE: begin
                    ...
                    if (tx_load_w) begin
                        ...
                        tx_bit_cnt  <= 3'd0;
                        tx_state    <= TX_START;
                        uart_tx     <= 1'b0;  // start bit begins on first TX_START cycle
                    end
                end
                TX_START: begin
                    uart_tx <= 1'b0;
                ...
                TX_DATA: begin
                    ...
                    if (tx_baud_tick) begin
                        ...
                        if (tx_bit_cnt == 3'd7) begin
                            tx_state <= TX_STOP;
                            uart_tx  <= 1'b1;  // stop bit begins on first TX_STOP cycle
                        end else
...
`ifdef FORMAL
    // ── 8N1 TX frame-boundary invariants ─────────────────────────
    always @(posedge clk) begin
        if (rst_n) begin
            // P_IDLE:  idle state → line is high (mark)
            assert(tx_state != TX_IDLE  || uart_tx == 1'b1);
            // P_START: start-bit phase → line is low (space)
            assert(tx_state != TX_START || uart_tx == 1'b0);
            // P_STOP:  stop-bit phase → line is high (mark)
            assert(tx_state != TX_STOP  || uart_tx == 1'b1);
        end
    end

    // ── IRQ pulse-width invariants ────────────────────────────────
    // tx_done_pulse and rx_complete_pulse must be exactly 1 cycle wide.
    // If these pulses were wider, the INTR_STATE register would be set
    // multiple times per event, causing spurious IRQ storms.
    always @(posedge clk) begin
        if (rst_n) begin
            // P_TX_PULSE: tx_done_pulse is deasserted one cycle after assertion
            if ($past(tx_done_pulse))
                assert(!tx_done_pulse);
            // P_RX_PULSE: rx_complete_pulse is deasserted one cycle after assertion
            if ($past(rx_complete_pulse))
                assert(!rx_complete_pulse);
        end
    end

    // ── RX FSM bit-counter bound ──────────────────────────────────
    // P_RX_BIT_CNT: rx_bit_cnt is always in [0..7] — the counter must
    // never wrap or overflow, which would cause incorrect byte assembly.
    always @(posedge clk) begin
        if (rst_n)
            assert(rx_bit_cnt <= 3'd7);
    end
`endif
