...
| metrics | tb_metrics (4 programs): prog_forwarding CPI=1.100 (1 load-use stall); prog_branch_pred per-test: T1=81.8%(loop), T2=16.7%(BTB cold), T3=68.4%(nested), T4=47.6%(adversarial), overall=57.9%(57 br); prog_ahb_sfr AHB latency avg=9.4 CPU cycles (min=9 max=10, 10 txn); prog_fib CPI=1.182 (18 load-use stalls, 80% branch hit) | **PASS** |
...
make metrics                   # IPC + stall breakdown + per-test branch hit rate + AHB latency (4 programs)
