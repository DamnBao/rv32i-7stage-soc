...
- AHB irq → **3×2-FF Synchronizer (1GHz, per-source, trong soc_top)** → PLIC → Zicsr
...
`if1_stage`, `if2_stage`, `id_decoder`, `register_file`, `alu`, pipeline registers (if1_if2_reg, if2_id_reg, id_ex_reg, ...), Hazard Unit, Forwarding Unit, Zicsr, **PLIC** (6 sources, 3-bit priority, SiFive spec; 3×2-FF sync cho AHB IRQs trong soc_top)
...
| `mem1_stage.sv` | Hoàn thành (thêm PLIC decode: addr[31:24]==0x0C, plic_re/we/addr/wdata, mem_src=2'b11) |
| `dmem.sv` | Hoàn thành |
| `mem1_mem2_reg.sv` | Hoàn thành (có load/store fault signals) |
| `mem2_stage.sv` | Hoàn thành (có load/store fault signals; thêm plic_rdata input, mem_src=2'b11 mux) |
...
| `zicsr.sv` | Hoàn thành (port meip_in từ PLIC thay thế ahb_irq/axi_irq; 6 CSR regs, vectored) |
...
| `soc_top.sv` | Hoàn thành (3×2-FF sync AHB IRQs per-source; PLIC instance; expose 3×AXI + 3×AHB slave ports) |
